...
public:
	...
	__declspec(property(put = SetMonth, get = GetMonth))short Month;//string
	...
	__declspec(property(put = SetYear, get = GetYear))short Year;//string
	...
	static short NumberOfDaysInAMonth(short Month, short Year)
	{
		...
		int NumberOfDays[12] = { 31,28,31,30,31,30
							   ,31,31,30,31,30,31 };
	...
	static	void  PrintMonthCalendar(short Month, short Year)
	{
		...
		{
			//Arrang The Group Of Blanks Under Each Other
			printf("%5d", j);
	...
	static short DaysFromTheBignningOfTheYear
	(short Day, short Month, short Year)
	{
	...
	short DaysFromTheBignningOfTheYear()
	{
	...
	(short DateOrderInYear, short Year)
	{
		clsDate Date;
	...
	void AddDays(short Days)
	{
		short RemainingDays = Days + 
			DaysFromTheBignningOfTheYear(_Day, _Month, _Year);
		short MonthDays = 0;
		...
		while (true)
		{
			...
			if (RemainingDays > MonthDays)
			{
				...
				//if month has arrived the number 12 than reduce
				//repated month to 1 than reduce year++
				if (_Month > 12)
				{
					_Month = 1;
					_Year++;
	...
	(clsDate Date1, clsDate Date2, bool IncludeEndDay = false)
	{
		int Days = 0;
		short SawpFlagValue = 1;

		//Date1 Is Small Than Date2 This Mean Date2 Is Greater
		...
		return IncludeEndDay ? ++Days * SawpFlagValue :
			Days * SawpFlagValue;
	}

	int GetDifferenceInDays
	(clsDate Date2, bool IncludeEndDay = false)
	{
		return GetDifferenceInDays(*this, Date2, IncludeEndDay);
	}

	static	int  CalculateMyAgeInDays(clsDate DateBirth)
	{
		return GetDifferenceInDays(DateBirth, clsDate::GetSystemDate(), true);
	}

	//above no need to  have nonstatic function for the
	// object because it does not depend on any data from it.
	...
	static	clsDate IncreaseDateByOneMonth(short Month, clsDate& Date)
	{
		if (Date.Month == 12)
		{
			Date.Month = 1;
	...
	void IncreaseDateByOneMonth(short Month)
	{
		IncreaseDateByOneMonth(Month, *this);
	}
	...
	static	clsDate IncreaseDateByXMonths(short Months, clsDate& Date)
	{
		...
		{
			Date = IncreaseDateByOneMonth(Months, Date);
		}
		return Date;
	}
	...
	static	clsDate DecreaseDateByOneMonth(short Month, clsDate& Date)
	{
		if (Date.Month ==1 )
		{
			Date.Month = 12;
	...
	void DecreaseDateByOneMonth(short Month)
	{
		DecreaseDateByOneMonth(Month, *this);
	}
	...
	static	clsDate DecreaseDateByXMonths(short Months, clsDate& Date)
	{
		for (short i = 1; i <= Months; i++)
		{
			Date = DecreaseDateByOneMonth(Months, Date);
		}
		return Date;
	}
	...
	static	short IsEndOfWeek(clsDate& Date)
	{
		...
		(Date.Day, Date.Month, Date.Year)== 6;
	}
	...
	static clsDate CalculateVacationReturnDate(clsDate DateFrom, short VactionDays)
	{
